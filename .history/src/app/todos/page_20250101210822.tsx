  ...
  return (
    <>
      <div className="grid place-items-center w-full lg:place-items-start text-purple-500 min-h-screen">
        ...
        <ul className="sm:w-9/12 lg:w-5/12 w-full px-4 flex flex-col justify-center  my-5 py-5">
        {isLoading && <p className="text-pink-500 text-xl italic my-6"> Loading...</p>}
        {!isLoading && todos && todos.length == 0 ? 
        (<div className="text-pink-500 text-xl italic my-6"> There is No thing </div>): 
        (
          <>
          {!isLoading && todos && todos.map((todo:Todo)=>(
            <li key={todo._id} className="bg-slate-900 p-6 rounded-lg my-2 hover:text-green-400 w-full text-lg"></li>
          ))}
          </>
        )}
        </ul>
