...
export default function NavBar() {
  return (
    <nav className=" dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b border-gray-200 ">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
          <Link href={Routes.auth.LOGIN}>
            <button
              type="button"
              className="text-white  hover:bg-blue-950 focus:ring-4 
          ...
          <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-2 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
            {MenuItems?.map((items) => (
              <li key={items.href}>
                <a
                  href={items.href}
                  className="block py-2 px-1 text-white rounded md:bg-transparent md:p-0 "
                  aria-current="page"
                >
                  {" "}
                  {items.name}{" "}
