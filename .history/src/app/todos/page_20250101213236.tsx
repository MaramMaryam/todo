  ...
  const hanldeSave=async()=>{
    if(!editTodo) return;
     ...
     if(response.status === 200){
      setTodos(
        todos.map((todo:Todo)=> todo._id === editTodo._id ? {...todo, text: editTodo.text}: )
      )
     }
  }
  return (
          ...
          <div className="sm:w-9/12 lg:w-6/12 w-full px-4 lg:my-10 flex flex-col justify-center items-center">
            ...
            {editTodo ? (
              <>
                ...
                />
                <button onClick={hanldeSave} className="bg-slate-800 border px-5 py-2 rounded-lg my-8 text-green-400 text-lg font-semibold"n>
                  save
                </button>
              </>
            ) : (
